...
def reset_token(token):
    ...
    if not user:
        flash('The token is invalid or has been expired', 'failure')
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Your password has now been reset, You can now login', 'success')
        return redirect(url_for('users.login'))
    return render_template('reset_token.html', titel='Reset Password', form=form)
